...
function MobHealth_PPP(	index )
	local hpData = MI2_MobHpDB[index]
	if hpData then
		local pts, pct = MI2_HpDecode( hpData )
...
-- MI2_AbbrevNumber()
--
-- Format a number as an abbreviated string with 2 decimal places.
-- e.g. 4250 -> "4.25k", 1200000 -> "1.20m"
-----------------------------------------------------------------------------
function MI2_AbbrevNumber( n )
	if n >= 1000000 then
...
function MobHealth_Display( )
	...
	if MI2_Target.showHealth then
		if  MobInfoConfig.TargetHealth == 1 and MI2_Target.maxHealth then
			if MI2_HpData.dbQuality == 3 then
				if MobInfoConfig.AbbrevHP == 1 then
					healthText = MI2_AbbrevNumber( MI2_Target.curHealth )
				else
					healthText = string.format("%d", MI2_Target.curHealth )
				end
			else
				if MobInfoConfig.AbbrevHP == 1 then
					healthText = MI2_AbbrevNumber( MI2_Target.curHealth )
	...
	if maxmana > 0 then
		...
		if MobInfoConfig.TargetMana == 1 then
			if MobInfoConfig.AbbrevHP == 1 then
				manaText = MI2_AbbrevNumber( mana )
			else
...
local function MI2_MobHealth_SetFont( fontId, fontSize )
	...
	if fontId ~= lOldFontId or fontSize ~= lOldFontSize then
		...
		MI2_MobHealthText:SetFont( fontName, fontSize, "OUTLINE" )
		MI2_MobManaText:SetFont( fontName, fontSize, "OUTLINE" )
	end
...
function MI2_MobHealth_Reset()
	...
	-- clear mh field from all MobInfoDB entries
	if MobInfoDB then
...
function MI2_SaveTargetHealthData( updateOnly )
	if MI2_Target.index and MI2_HpData and MI2_HpData.newMax then
		...
		-- only save if the corresponding recording option is enabled
		local isMobHp = (MI2_Target.healthDB == MI2_MobHpDB)
		if isMobHp and MobInfoConfig.SaveMobHp == 0 then return end
		if not isMobHp and MobInfoConfig.RecordPlayerHp == 0 then return end
