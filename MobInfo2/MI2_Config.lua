...
-----------------------------------------------------------------------------
-- MI2_EnableChildCheckBox() / MI2_DisableChildCheckBox()
--
-- Enable or disable a child checkbox and grey/restore its label text,
-- matching the pattern used by FlightMap and the WoW OptionsFrame API.
-----------------------------------------------------------------------------
function MI2_EnableChildCheckBox( checkbox, checked )
...
function MI2_DisableChildCheckBox( checkbox )
	if not checkbox then return end
	checkbox:Disable()
	local text = getglobal( checkbox:GetName().."Text" )
...
function MI2_UpdateOptions()
	...
	-- enable/disable SavePlayerHp child based on RecordPlayerHp parent state
	if MobInfoConfig.RecordPlayerHp == 1 then
		MI2_EnableChildCheckBox( MI2_OptSavePlayerHp, MobInfoConfig.SavePlayerHp )
	else
		MI2_DisableChildCheckBox( MI2_OptSavePlayerHp )
	end
end
...
-----------------------------------------------------------------------------
-- MI2_OptRecordPlayerHp_OnClick()
--
-- When the "Record Player Health Data" parent checkbox is toggled,
-- enable or disable the "Save player health data permanently" child.
-----------------------------------------------------------------------------
function MI2_OptRecordPlayerHp_OnClick()
	if MobInfoConfig.RecordPlayerHp == 1 then
		MI2_EnableChildCheckBox( MI2_OptSavePlayerHp, MobInfoConfig.SavePlayerHp )
	else
		MI2_DisableChildCheckBox( MI2_OptSavePlayerHp )
...
function MI2_DbOptionsFrameOnShow()
	...
	-- count and display size of mob health data (mh fields in MobInfoDB)
	...
	if MI2_Target.mobIndex and MobInfoDB[MI2_Target.mobIndex] then
		...
		MI2_TxtTargetIndex:SetText( MI_TXT_CUR_TARGET..mifontWhite..MI2_Target.name..":"..MI2_Target.level )
	else
